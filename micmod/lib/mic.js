    ...
    that.start = function start() {
          if(audioProcess === null) {
            ...
            audioProcess.stdout.on('readable', () => {
                let chunk;
                while (null !== (chunk = audioProcess.stdout.read(chunkSize))) {                    
                    if(isWindows)
                    {                    
                        const dataBuf = Buffer.alloc(chunk.length / 2);
                        for (i = 0; i < dataBuf.length; i+=2)
                        {
                            var v = chunk.readInt32LE(i * 2) >> 16;
                            //v = Math.min(Math.max(v * 10, -32768), 32767);
                            dataBuf.writeInt16LE(v, i);
                        }
                        chunk = dataBuf                        
                    }
